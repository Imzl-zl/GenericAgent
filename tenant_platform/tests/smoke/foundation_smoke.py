...
def _write_fixture_config(config_root: Path, fixture_url: str) -> None:
    config_root.mkdir(parents=True)
    path = config_root / "mykey.py"
    path.write_text(
        "native_oai_config = {\n"
        "    'name': 'foundation-smoke-fixture',\n"
        f"    'apikey': {OAI_TOKEN!r},\n"
        f"    'apibase': {fixture_url!r},\n"
        "    'model': 'gpt-test',\n"
...
def _launch_stack(
    ...
    runtime_root.mkdir()
    _write_fixture_config(config_root, fixture.server.base_url)
