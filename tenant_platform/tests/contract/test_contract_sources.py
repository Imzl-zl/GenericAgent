...
def test_llm_proxy_contract_declares_versioned_service_and_no_api_key():
    ...
    """
    text = (ROOT / "contracts/proto/genericagent/proxy/v1/llm_proxy.proto").read_text(encoding="utf-8")
    ...
    lowered = text.lower()
    for forbidden in ("api_key", "apikey", "api_key_id", "secret", "secret_key", "bearer_token"):
        assert forbidden not in lowered, f"llm_proxy.proto must not declare {forbidden!r}"
...
def test_platform_openapi_declares_error_response_schema_and_status_enum():
    ...
    for value in ("queued", "starting", "running", "succeeded", "failed", "cancelled", "interrupted"):
