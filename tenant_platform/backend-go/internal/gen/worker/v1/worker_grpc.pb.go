...
func (c *workerServiceClient) ExecuteTask(ctx context.Context, in *ExecuteTaskRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[WorkerEvent], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &WorkerService_ServiceDesc.Streams[0], WorkerService_ExecuteTask_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[ExecuteTaskRequest, WorkerEvent]{ClientStream: stream}
...
type WorkerService_ExecuteTaskClient = grpc.ServerStreamingClient[WorkerEvent]
...
func _WorkerService_ExecuteTask_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(WorkerServiceServer).ExecuteTask(m, &grpc.GenericServerStream[ExecuteTaskRequest, WorkerEvent]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type WorkerService_ExecuteTaskServer = grpc.ServerStreamingServer[WorkerEvent]
