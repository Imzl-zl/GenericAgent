...
**业界对照（Coze 官方文档，2026-08-10 调研）：**

- Coze 多会话核心特性原话：**"文件共享，上下文隔离"**——所有会话作用于同一个项目/工作空间，文件实时共享；每个会话独立对话上下文。
- 会话隔离层级：`渠道 + 账号 + Bot + session_name`；会话/对话/消息是全局资源，**不隶属于工作空间**。
- 群聊：Coze 飞书渠道支持"单聊深度对话，也可在群内协同辅助"——群就是一个独立的对话单元。
- 结论：**群聊与个人不区分工作空间**（业界如此，区分成本高收益低），隔离的是会话/上下文，不是工作空间。本项目模型与业界一致。
