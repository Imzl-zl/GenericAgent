...
- **/new 会话语义（2026-08-15 定案）**：/new = 对话历史 + 会话文件引用双清——任务注入的 Recent session files 只含本桶 reset_at 之后文件（RecentSince 过滤）；/new 物理清理旧会话 inbound 附件（PruneInboundBefore），outbound 产物保留磁盘但不注入。旧行为（只清历史）曾致新会话任务误转旧文件。边界：文件池为工作区级共享，隔离是时间级过滤而非按桶归属（多桶渠道下其他桶 reset 后导入的文件仍可能注入；微信单桶无影响）。
- **poller 入站合并窗口 = 配置对账（2026-08-15 定案）**：窗口真值源 = DB platform_runtime_settings；ReconcileBots 每 tick 对比 /health 上报的窗口值，不一致才重推 POST /config（poller 重启后一个 tick 内恢复，曾静默失效 18h）；旧版 poller 不报该字段（=0）仍会触发推送，兼容。改窗口相关代码注意三处同步（BotManager/provider、platform 对账、/health 字段）。
...
- **生图 image_gen（2026-08-14，Phase B v1 直连形态）**：第 10 原子工具；llmcore `ImageGenClient`（实现进 llmcore.py，**严禁新增 imagegen.py**——沙箱 overlay 只物化固定清单 LEGACY_MODULES，新增模块 = 平台沙箱启动 ImportError 全平台任务失败）；配置 = mykey `image_gen` dict（变量名不得含 api/config/cookie 子串，agentmain 会话扫描误判）；**错误前缀用 `[Error: image_gen ...]`，绝不用 `!!!Error:`**（模型最终回复尾部含它触发 do_no_tool 致命判定/LLM_FAILED）；**response_format 只给 dall-e 发**（gpt-image 恒 b64_json，发该参数可能 400，同步默认路径就废）；**落盘必须走 self.cwd/outputs/**（硬编码 script_dir 在容器里写错位置，交付链断）；落盘前 ≤20MiB 检查（Go 交付上限 fail-closed）；文件名带时间戳（`image_<ts>_<i>.png`）防同任务重复调用同名覆盖；n>1 返回多行 `[FILE:]` marker；**marker 回显依赖**——模型须在最终回复回显 `[FILE:outputs/...]` 才触发交付（直连形态无兜底；**托管形态已补 generated_output_files 登记**：worker `install_image_gen_marker_registry` 包装 do_image_gen，终态 append_missing_file_markers 自动补写）；双形态设计：直连 = 真实上游+密钥，托管 = llm-proxy + llm.image 能力令牌，一份代码配置决定。**new-api 中转实测结论（2026-08-14，newapi.myovo.cc.cd）**：①**size 必传**（计费硬要求，缺省 500"图片尺寸计费需要传 size"）——客户端已默认 1024x1024；②gpt-image-2/gemini-3-pro-image/gemini-3.1-flash-image-preview 返回 b64_json（quality/output_format 兼容）；③**sensenova-u1-fast / agnes-image-2.1-flash 只回 `url` 直链**（b64_json 空）——客户端已加 url 直下兜底；④sensenova size 合法集特殊（1664x2496/2048x2048 等，无 1024x1024），错误文本含合法列表模型可自愈；⑤流式（stream:true+partial_images SSE）未实测，建议 stream=False 走同步。真实 CLI 闭环验证通过（agnes-2.5-flash 对话 + gpt-image-2 生图）。**参数协商自愈实测升级（2026-09-13，newapi.myovo.cc.cd → Agnes 2.5）**：⑥**"OpenAI 兼容"≠ 参数全集通用——上游按队列裁剪参数**：agnes-image-2.5-flash 的 text image queue 只收 `model/prompt/size/ratio/extra_body`，收到 `output_format`/`quality` 直接 400 `X is not supported by text image queue`（而同网关 gpt-image-2 恰好接受 output_format）→ 客户端已实现**错误文本驱动的参数协商**（`_post` 裁剪循环 + `_unsupported_param` 三重门槛：话术族命中 + 参数名字面出现 + 确在本次 payload；`_IMAGE_GEN_TRIMMABLE` 只含 output_format/quality/response_format/stream/partial_images，**刻意不裁 size/n**——静默缩水不诚实；预算 3 次不消耗 max_retries；预算尽则如实回上游 message）；⑦**`png` 是协议默认输出格式 → 不再显式发**（显式发只是多一个 400 借口），仅 webp/jpeg 才声明；⑧**落盘扩展名按魔数嗅探**（`sniff_image_format`，协商裁掉 output_format 后请求 jpeg 可能拿回 png，旧实现会写出"名叫 .jpeg 的 png" → IM MIME 失配），ga.py 嗅探失败才回退请求值、不符时 `[Status] ℹ️` 明示；⑨**渠道 key 与模型必须配对**：渠道1 key 只有 6 个模型（无 agnes），agnes 模型 + 渠道1 key = 503 `model_not_found ... under group image`（与参数 400 完全不同的故障面，排障先 `GET /v1/models` 自查）；⑩**网关在 Cloudflare 后、proxy read timeout=120s**：gpt-image-2 实测 24-125s 会撞 **524**（HTML 响应体），agnes 8-11s 安全 → 默认生图模型已改 agnes-image-2.5-flash、`read_timeout=100`（< CF 窗口，宁可自己超时也不吃 CF HTML）；⑪**agnes 的 `response_format` 必须放 `extra_body.response_format`，顶层放是官方明示的错误写法**；`return_base64` 经该中转被忽略（仍回 url，走既有 url 直下兜底）；`ratio`（1:1/16:9/21:9…）是独立顶层参数，当前工具 schema 未暴露（需 size 档位 1K/2K/3K/4K 配合，列为后续增强）。⑫**生图超时必须按「入口窗口 / 任务预算」倒推，不能只填协议默认值**：入口是 Cloudflare 时 proxy read timeout=120s——客户端 read_timeout 若不显式下发就会取协议默认 120s，正好顶到 CF 窗口，慢模型拿到的是**不可判读的 HTML 524**；而单任务硬预算 `TASK_TIMEOUT_SECONDS=300`（compose GA_RUNNER_TASK_TIMEOUT）要求 `(max_retries+1) × read_timeout < 300s`，否则生图会把任务预算烧光 → **TASK_INTERRUPTED 零回复**（与 08-13 图片任务超时同源）。定稿值：托管块 `read_timeout=100`（env `GA_IMAGE_GEN_READ_TIMEOUT` 可覆盖）+ `max_retries=1` → 最坏 ≈202s；本地 mykey 同值。两条不变式有测试钉住（`TestBuildRuntimeConfigImageGenTimeoutBoundary`）。验证：根 136 全绿（test_image_gen 30→44）、真实 key 单请求 9.6s 出图、Go 全量 + race 绿。⑬**托管形态下「错误文本驱动自愈」会被安全边界吃掉（2026-09-13 生产实测；用户可见后果：模型盲重试 4 次后放弃、回「图片服务不可用」）**：llm-proxy 默认清洗上游错误体（安全边界：避免账号/配额泄露，排障看代理日志），GA 只收到 `{code:UPSTREAM_ERROR}` → **任何靠错误文本解析的自愈在托管形态下失效**。修法两侧互补：①代理侧对**生图端点 4xx 参数类白名单话术**只提取 `message` 重建后透传（不原样转发、兄弟字段不出门、5xx/chat 仍清洗）；②客户端侧加**保守集兜底**（4xx 不可判读且仍带装饰性参数 → 丢弃 `_IMAGE_GEN_TRIMMABLE` 重试一次）。**通用教训**：让客户端依赖「上游错误文本」做决策前，必须先确认链路上有没有清洗/改写层（反向代理、网关、SDK），否则自愈逻辑只在直连时成立。⑭**改图/参考图 image.edit 已实测可用，且是「通道属性」不是模型属性（2026-09-13 源码 + 实测坐实）**：`POST /images/edits`（multipart，文件字段 `image`）+ `gemini-3.1-flash-image` → 200/9.0s，像素验证参考图生效。**new-api 丢弃自定义参数的机制已定位到源码**：`relaykit/dto/openai_image.go` 的 `UnmarshalJSON` 把未知字段收进 `Extra`，而 `MarshalJSON` 合并 Extra 的循环**被注释掉**（旁注：不能合并 ExtraFields）→ `extra_body`/`ratio`/`return_base64` 一律静默失效；能过网关的图像参数只有 DTO 字段表里那些；edits 表单只读 `formData.Get("image")`（**单值**，多图未实现）。**落地**：客户端 `protocol`+`operations` 声明能力、**端点按 operation 选**、未声明 **fail-closed**；工具新增 `image` 参数（读盘/路径安全/魔数校验在工具层）；`image_edit` 与 `image_gen` 分开配置（免费文生图 vs 付费改图）。**方法论**：能力矩阵必须记录探测所用 key/分组（本轮曾因 key 用错误判「无可用渠道」）；「200 就算成功」不可信，须用像素/内容客观验证 —— 详见 `.tasks/image-capability/DESIGN.zh-CN.md`。⑮**SenseNova U1.5 Lite（免费改图，2026-09-13 官方文档 + 实测）**：`model=sensenova-u1.5-lite` 生成与编辑一体；改图 `POST /v1/images/edits` 是 **JSON**（非 multipart）+ `images:[{image_url: <公网URL 或 data:image/*;base64,...>}]`，**不接受纯 base64**；`n` 只能 1；`size` 支持 2K/4K 常量（32 倍数、≤4096、比例 ≤3:1）；`watermark` 默认 true（`false`=公测期免费去水印，建议显式传）；`prompt_extend` 默认 true（内置提示词润色）。实测：改图 200/**49.6s**（像素验证生效）；**2K 文生图撞 CF 524（>120s）→ CF 后面别用 2K**。客户端对应 `protocol: images_edits_json` + `extra_params`（不得覆盖语义参数）。**形态选错就是"参数差异"类报错**（multipart↔JSON 互换被上游回 invalid arguments）。设计真值：`.tasks/im-media-pipeline/PHASE_B_IMAGE_GEN_PLAN.zh-CN.md`（§9.9）
...
- **推送方式（2026-09-13 定案）**：`origin` 用 **SSH**（`git@github.com:Imzl-zl/GenericAgent.git`，`~/.ssh/id_ed25519` 已注册）。原 HTTPS remote + `credential.helper=store` 的 `~/.git-credentials` 已空 → 非交互 push 直接 `Invalid username or token`；HTTPS 路径需重新配 token，非交互场景一律走 SSH。
- **部署前先对账「本地 vs 远端」**（2026-09-13 实证）：本机 clone 曾停在 10 天前而远端 main 已前进 20+ 提交、线上镜像是更早的构建 → 直接 `make build` 会把多天工作**静默带上线或回退生产**。正规流程：`git fetch` → 比对 `HEAD..FETCH_HEAD` → rebase 到远端 tip → 用 `docker images` 的 `:<git describe>` tag 反查线上实际构建版本 → 再 build/deploy。
- **部署前必备份 tag**：`docker tag genericagent-<svc>:local genericagent-<svc>:local.bak-<YYYYMMDD>`（六服务）+ 记录旧 `GA_RUNNER_IMAGE` digest；回滚 = 重打 `:local` + 还原 `.env` digest + `docker compose up -d`。**绝不用 `docker image prune -a`**（会删所有未被容器引用的镜像，含追溯 tag 与备份，2026-08-14 踩过）。
- **镜像 ID 在 containerd 镜像存储下不是内容稳定标识（2026-09-13 实证）**：本机 Docker 29.7.2 用 `io.containerd.snapshotter.v1`，**内容完全相同的两次构建会得到不同 image ID**（`docker image inspect` 的 Config/History/Created/RootFS 逐字段相同，只有 ID/`Metadata.LastTagTime`/containerd `Ref` 不同）。后果：①`make build` 后**必须重跑 `make runner-digest`** 更新 `.env`，否则 `.env` 指向的是上一次构建的 ID（内容相同也能跑，但对账口径不一致）；②**绝不能 pruned 掉 `.env`/运行中引用的镜像**（`GA_RUNNER_IMAGE` 是 ID 引用，镜像没了 = WORKER_START_FAILED，08-14 踩过）；③判断「这次部署有没有生效」不能看镜像 ID 变没变，要看**内容**（见下条）。
- **验证「修复真的进了部署产物」的两个可信姿势（2026-09-13 定型）**：①平台二进制：`docker cp <platform容器>:/opt/ga/bin/platform /tmp/p && grep -c -a '<符号/字符串>' /tmp/p`（容器内**没有 `strings` 命令**，直接 `docker exec ... strings` 会静默返回 0 造成假阴性——先拿一个已知字符串自检方法有效性）；②ga-runner 代码：`docker run --rm --entrypoint sh genericagent-ga-runner:local -c 'grep -c <符号> /ga/legacy/llmcore.py'`（沙箱代码是 COPY 进 /ga 的，可直接 grep）。反向判据同样重要：`docker compose up -d` 只重建**镜像内容有变化**的容器，没重启 ≠ 部署失败。
- **构建产物相同的镜像不会被重启**：`docker compose up -d` 只重建镜像层有变化的容器（如仅改 runtime_config 时 ga-runner/web/bot-poller 复用旧 ID、不重启）——别把「没重启」当部署失败，用 `docker inspect <c> --format '{{.Image}}'` 对 ID 验证。
