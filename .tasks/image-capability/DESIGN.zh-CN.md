...
2. **参考图/改图当前不可用**，且是**三层叠加**造成的，不是"没配"：
   - 上游 Agnes 的改图协议是 `extra_body.image`（官方文档），不是 `/images/edits`；
   - 我们的网关（new-api v1.0.0-rc.33）图片中继**只转发它 DTO 内声明的字段**，`extra_body`/`ratio`/`return_base64` 被**静默丢弃**；
   - new-api 虽**有** `/v1/images/edits` 路由与多渠道路由（官方文档 + 源码 + 相关 PR），但在本实例里**没有可用渠道**（`abilities` 表无对应图像模型；上游对 edits 端点回 `no available server`）。
...
| `/v1/images/edits`（agnes 渠道） | ❌ 无上游 | 503 `no available server`（106s 后） |
| `/v1/images/edits`（gemini/gpt-image 模型） | ❌ 无可用渠道 | 503 `No available channel for model ... under group free (distributor)` |
...
**new-api 侧事实**（版本 `v1.0.0-rc.33`，revision `eb99ab1b`，本地容器 `new-api`，即用户的 CF 隧道网关）：
...
`BaseImageGenClient` → `OpenAIImageGenClient`（现有）；改图接入时新增 **`OpenAIImageEditClient`**（multipart `/v1/images/edits`），配置用 `protocol: "images_generations" | "images_edits"` 分派（`resolve_image_gen` 已按 `kind` 分派）。
**必须遵守的两条新规则（本次实测换来的）**：
...
| **P1（待决策）** | 改图/参考图能力：接入一条**真正支持 edits 的上游**（官方 Gemini image edit / OpenAI gpt-image edits / 豆包 seedream / xAI grok-imagine，new-api 均已适配）→ 按 §4.1/§4.2 扩 operation + `OpenAIImageEditClient` | **用户选渠道**（账号/成本） | 契约 + Go + worker + web 同步；`/images/edits` 真实出图；`operations` 未声明时 fail-closed |
...
1. **改图方向**：接入支持 edits 的上游（推荐，能力真、复用现有网关/计费/监控）还是**接受"无改图"**（用合成替代，零成本）？
